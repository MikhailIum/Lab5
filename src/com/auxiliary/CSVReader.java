...
public class CSVReader {
    ...
    final static private String FILE_PATH = System.getenv("FILEPATH");
    ...
    private String getName(CSVReader r, List<CSVReader> list, String type, boolean admin, boolean location){
        String name = type;
    ...
    private Coordinates getCoords(CSVReader r, List<CSVReader> list) throws IOException {
        ...
        else cords = new Coordinates(Double.parseDouble(r.x), Long.parseLong(r.y));
        return cords;
    }

    private String getStudentsCount(CSVReader r, List<CSVReader> list, String studentsType, String typeName) throws IOException {
    ...
    private Location getLocation(CSVReader r, List<CSVReader> list, LinkedList<StudyGroup> groups) throws IOException {
        ...
        return new Location(Float.parseFloat(locationX), Integer.parseInt(locationY), Long.parseLong(locationZ), locationName);
    }


    private Person getAdmin(CSVReader r, List<CSVReader> list, LinkedList<StudyGroup> groups) throws IOException {
    ...
    private void getStudyGroup(CSVReader r, List<CSVReader> list, LinkedList<StudyGroup> groups) throws IOException {
        ...
        String expelledStudents = r.getStudentsCount(r, list, r.expelledStudents, "Number of expelled students");
        String shouldBeExpelled = r.getStudentsCount(r, list, r.shouldBeExpelled, "Number of students who should be expelled");
        Semester semester = r.getSemester(r, list);
        Person admin = getAdmin(r, list, groups);
        groups.add(new StudyGroup(name, cords, Integer.parseInt(studentsCount), Integer.parseInt(expelledStudents), Long.parseLong(shouldBeExpelled), semester, admin, LocalDateTime.parse(r.creationDate), r.id));
    }
    ...
    public static LinkedList<StudyGroup> makeCollection(List<CSVReader> list) throws IOException {
        ...
        for (CSVReader r : list) {
            if (r.location_name == null){
                System.out.println(TextColor.ANSI_RED + "Line " + (list.indexOf(r) + 1) + " in file " + FILE_PATH + "have not enough parameters");
                System.exit(0);
            }
