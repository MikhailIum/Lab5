...
public class AddCommand extends Command {
    ...
    /**
     * Allows user to set the name of the new group or admin's name or location's where admin is from name
     */
    ...
    /**
     * Allows user to set the number of students of the new group(or expelled students or those who should be expelled)
     * @param field - Number of students/Number of expelled students/Number of students who should be expelled
     */
    public static Integer getStudentsCount(String field) throws IOException {
        System.out.print(TextColor.ANSI_PURPLE + field + TextColor.ANSI_BLUE + "(positive integer)" + TextColor.ANSI_PURPLE + ": " + TextColor.ANSI_RESET);
        String num;
    ...
    public static LocalDate getBirthday() throws IOException {
        System.out.print(TextColor.ANSI_PURPLE + "Birthday" + TextColor.ANSI_BLUE + "(DD.MM.YYYY)" + TextColor.ANSI_PURPLE +": " + TextColor.ANSI_RESET);
        String date;
    ...
    public static String getCoordinate(String coord) throws IOException {
        String type = "";
        if (Objects.equals(coord, "x"))
            type = "(double)";
        else if (Objects.equals(coord, "y") || Objects.equals(coord, "z"))
            type = "(integer)";
        System.out.print(TextColor.ANSI_PURPLE + coord + TextColor.ANSI_BLUE + type + TextColor.ANSI_PURPLE + ": " + TextColor.ANSI_RESET);
        String x;
    ...
    private StudyGroup getStudyGroup(String name, boolean isNameSet) throws IOException {
        ...
        return new StudyGroup(name, coords, studentsCount, expelledStudents, shouldBeExpelled, semester, getAdmin(), LocalDateTime.now(), UUID.nameUUIDFromBytes(name.getBytes()));
    }
    ...
    public void execute(Client client, String[] args) throws IOException {
        ...
        if (args.length == 3){
            if (Objects.equals(args[1], "if min")){
                if (args[2].compareTo(client.groups.getFirst().getName()) > 0){
                    System.out.println(TextColor.ANSI_YELLOW + "This isn't the lowest name\n" + TextColor.ANSI_RESET);
